...
- The shared temperature palette refreshes automatically when its contract changes, with four-way provider fan-out, bounded producer handoffs, and five-attempt producer retries. The shared 500-mb style refresh uses the same safeguards for both North America and Northern Hemisphere views. Self-publishing providers now pass a style-refresh flag, upload their completed run IDs, and use one serialized Pages handoff after the matrix finishes; providers with the normal workflow-run publisher retain that path. Corrected snowfall drains the same queue before dispatching its validated artifact to the shared publisher, so a pending Pages run cannot silently drop the snowfall update. The repair workflow is manual-only so one edit cannot launch duplicate fan-outs; operational schedules remain unchanged.
...
| `height-style-refresh.yml` | Automatic refresh on shared 500-mb style edits; four-way producer matrix, bounded waits, retries, and one serialized handoff for self-publishing NA and NH views. |
...
| `temperature-style-refresh.yml` | Automatic shared temperature-style refresh on palette edits; four-way provider matrix, product-scoped producers, bounded waits, retries, and one serialized handoff for self-publishing providers. |

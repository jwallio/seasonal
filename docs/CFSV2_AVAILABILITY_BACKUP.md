...
GitHub continues to download, validate, render, and publish maps. The Windows task only dispatches the availability workflow when it has no active run and no successful check in the past 30 minutes. It runs every 15 minutes, independently of GitHub cron.

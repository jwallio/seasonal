...
Only explicitly verified archive gaps listed in cfsv2_surface_schedule.py are omitted, on both sides of the comparison. Historical AWS objects whose companion .idx files are absent are recovered through validated bounded-range extraction; an object is not treated as missing until all five required surface fields fail validation. Unknown missing data cause acquisition failure. Publication requires all 15 reference years, both products, every winter period, and the matching cycle count. Failure or unavailable forecasts leave the previous corrected maps live. General weather publication preserves corrected snowfall independently.

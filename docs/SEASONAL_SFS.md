...
`kg m-2`, which is liquid-water equivalent. It is converted to LWE inches for
the anomaly calculation and rendered through the shared fixed 10:1 display
conversion. The adapter does not treat `swe` / snow-water equivalent as
snowfall.

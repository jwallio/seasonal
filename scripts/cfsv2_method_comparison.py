...
if a.year:
    cycle=str(a.year)+a.init[4:]
    arrays={}; sources=[]
    for target in months:
        ht=str(a.year+int(target[:4])-int(a.init[:4]))+target[4:]
        build_month(Path('.cache/compare/raw'),Path('.cache/compare/months'),cycle,ht)
        ...
        for method,values,lons,lats in [
            ...
            arrays[f'{method}_{target}']=values
    ...
    report={'init':a.init,'cycles':1,'reference_initialization_years':list(range(2011,2026)),'period':'DJF','ratio':10,'methods':{}}
    for method in ['native','phase']:
        ...
        cf.render_map(grid,a.init,months[0],3,[1],root/f'{method}-DJF.jpg',True,'2011–2025 matched operational reference',borders,
            period_label='DJF '+a.init[:4]+'–'+str(int(a.init[:4])+1)[-2:],seasonal=True,ensemble_label='Single initialization · member 1',product_spec=spec)
